...
export default function Shop() {
    ...
    const filteredProducts = products.filter((product) => {
        ...
        return (filters.search === null || (isIncluding(product.name, filters.search) || isIncluding(filters.search, product.name)) || (isIncluding(product.category, filters.search) || isIncluding(filters.search, product.category))) &&
            (filters.category === null || isIncluding(product.category, filters.category)) &&
    ...
    return (
        <div className='flex flex-col p-7 gap-7'>
            ...
            <div className={`fixed justify-center flex-col inset-y-0 pt-24 w-full ${toggleFilter? "right-0" : "-right-full"} transition-all flex box-border px-6 bg-white max-w-screen-md z-50`}>
                <span className={`${toggleFilter ? "opacity-100" : "opacity-0 pointer-events-none"} transition-opacity fixed bg-white inset-0 w-screen h-16 flex items-center shadow-sm`}>
            ...
            <span className='search flex rounded-xl bg-gray-100 items-center justify-around px-2 py-1 w-11/12 max-w-xs self-center'>
                {filters.search ?
                    <Image onClick={() => {
                        setSearch(null)
                        //ON grap le input de recherche pour le vider (visuellement)
                        document.getElementById('search').value = '';
                    }} className={'h-8 w-8 opacity-60 rotate-45'} src={require('../assets/images/increase.webp')} alt={'Réinitialiser la recherche'}/>
                :
                ...
                <input id={'search'} type={'search'} value={filters.search} placeholder={'Chercher un produit...'} onChange={(e) => setSearch(e.target.value)} className='bg-transparent placeholder-gray-400 py-2.5 text-md outline-none'/>
